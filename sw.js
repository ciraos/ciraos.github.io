...
(() => {
    /** 缓存库名称 */
    const CACHE_NAME = 'CiraosBlogCache'
    /** 控制信息存储地址（必须以`/`结尾） */
    const CTRL_PATH = 'https://id.v3/'

    const ejectDomain = 'blog.ciraos.top'
    ...
    request.url.startsWith('https://twikoo.ciraos.top') ||
    request.url.startsWith('https://meting.qjqq.cn') ||
    request.url.startsWith('https://api.i-metro.com') ||
    request.url.startsWith('https://friends.ciraos.top')
let cacheRules = {
simple: {
clean: true,
search: false,
